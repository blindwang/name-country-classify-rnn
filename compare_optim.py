...
for model_name in models_ls:
    ...
    for opt in optimizers:
        ...
        earlystop = EarlyStopping(patience=7, model_name=model_name + '_' + opt['name'] +'_bs_8')

        print(f"Training {model_name}... w/ {opt['name']}")

        """tensorboard writer"""
        train_summary_writer, test_summary_writer = bulid_tensorboard_writer(f"compare_optim_bs_8/{model_name}", opt['name'])
